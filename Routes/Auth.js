...
router.put('/updateProfileImage', async (req, res) => {
    ...
    if (!userId || !imageUrl) {
        return res.status(400).json({ ok: false, error: 'User ID and image URL are required' });
    }
    ...
    try {
        ...
        res.json({ ok: true, data: user, message: 'Profile image updated successfully' });
    } catch (error) {
        console.error('Error updating profile image:', error);
        res.status(500).json({ ok: false, error: 'Error updating profile image' });
    }
});
